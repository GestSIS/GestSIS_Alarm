...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--server",
            type=str,
            default=os.environ.get("GESTSIS_ALARM_MAIL_SERVER", None),
            help="Url of the mail server",
        )
        parser.add_argument(
            "--port",
            type=int,
            default=os.environ.get("GESTSIS_ALARM_MAIL_PORT", None),
            help="Port of the mail server",
        )
        parser.add_argument(
            "--username",
            type=str,
            default=os.environ.get("GESTSIS_ALARM_MAIL_USERNAME", None),
            help="Can be a username or your email address depending on your email provider",
        )
        parser.add_argument(
            "--password",
            type=str,
            default=os.environ.get("GESTSIS_ALARM_MAIL_PASSWORD", None),
        ...
        parser.add_argument(
            ...
            nargs="+",
            type=str,
            default=os.environ.get("GESTSIS_ALARM_MAIL_WHITELIST", None),
    ...
    def handle(self, *args, **options):
        for option in self._required_options:
            if not options[option]:
                raise ValueError(
                    "'{}' cannot be empty or None ! Specify it with the command line or in the .env file !".format(
                        option
                    )
                )
        ...
        logger.info(
            "Check on mail server {} downloaded {} new files".format(
                options["server"], len(pdf_downloaded)
            )
        )
